...
var ESLintPlugin = /** @class */ (function () {
    function ESLintPlugin(state) {
        ...
        if (state.standardPackagePath != null) {
            ...
            this.standardLinter = eslint_common_1.requireInContext(standardPackagePath, state.packageJsonPath);
        ...
        eslintPackagePath = eslint_common_1.normalizePath(eslintPackagePath);
        this.options = eslint_common_1.requireInContext(eslintPackagePath + "lib/options", state.packageJsonPath);
        this.cliEngineCtor = eslint_common_1.requireInContext(eslintPackagePath + "lib/api", state.packageJsonPath).CLIEngine;
    }
    ESLintPlugin.prototype.onMessage = function (p, writer) {
        ...
        try {
            ...
            response.isNoConfigFile = "no-config-found" === e.messageTemplate
                || (e.message && eslint_common_1.containsString(e.message.toString(), "No ESLint configuration found"));
...
function isFileKindAcceptedByConfig(config, fileKind) {
    ...
    function hasParser(parser) {
        return (config.parser != undefined && config.parser != null && eslint_common_1.containsString(eslint_common_1.normalizePath(config.parser), parser))
            || (config.parserOptions != undefined && config.parserOptions != null
                && eslint_common_1.containsString(eslint_common_1.normalizePath(config.parserOptions["parser"]), parser));
    }
...
function findESLintPackagePath(standardPackagePath, contextPath) {
    var resolvedStandardPackagePath = eslint_common_1.requireResolveInContext(standardPackagePath, contextPath);
    var requirePath = require.resolve("eslint", { paths: [resolvedStandardPackagePath] });
    requirePath = eslint_common_1.toUnixPathSeparators(requirePath);
